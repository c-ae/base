...
Module Go.
    ...
    := let bitwidths_used := ToString.bitwidths_used infos in
       ...
       strip_trailing_newlines
         (((if newline_before_package_declaration then [""] else [])
            ...
            ++ (let typedefs
                    := let carry_bitwidth := 64 (* c.f. https://github.com/REDACTED/fiat-crypto/pull/1006#issuecomment-892625927 *) in
                       let carry_typedef_comment := (" // We use uint" ++ Decimal.Z.to_string carry_bitwidth ++ " instead of a more narrow type for performance reasons; see https://github.com/REDACTED/fiat-crypto/pull/1006#issuecomment-892625927")%string in
                       List.flat_map
                         (fun bw
  ...
  (* (Comment modified from) REDACTED: In fiat-crypto C functions are void
